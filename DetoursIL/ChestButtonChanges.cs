...
public class ChestButtonChanges : DetourGroup
{
        ...
        if (ITDChestTE.IsActiveForLocalPlayer)
        {
            ...
            for (int num = 57; num >= 0; num--)
            {
                if ((num < 50 || num >= 54) && (inventory[num].type < ItemID.CopperCoin || inventory[num].type > ItemID.PlatinumCoin))
                {
                    ...
                    else if (inventory[num].stack == 0 || inventory[num].netID == 0 || inventory[num].type == ItemID.None)
                    {
                        list2.Add(num);
                    }
                }
    ...
    private static void DepositAllITDChest(On_ChestUI.orig_DepositAll orig, ContainerTransferContext context)
    {
        if (ITDChestTE.IsActiveForLocalPlayer)
        {
            Player player = Main.LocalPlayer;
