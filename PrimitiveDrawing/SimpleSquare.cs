...
namespace ITD.PrimitiveDrawing
{
    public class SimpleSquare
    {
        ...
        public void Draw(Vector2 positions, Color colors = default, Vector2 size = default, float rotation = 0, Vector2 rotationCenter = default)
        {
            ...
            vertices[1].Position = (positions + new Vector2((float)size.X * 0.5f, (float)-size.Y * 0.5f)).RotatedBy(rotation, rotationCenter).ToVector3();
            vertices[2].Position = (positions + new Vector2((float)-size.X * 0.5f, (float)size.Y * 0.5f)).RotatedBy(rotation, rotationCenter).ToVector3();
            vertices[3].Position = (positions + new Vector2((float)size.X * 0.5f, (float)size.Y * 0.5f)).RotatedBy(rotation, rotationCenter).ToVector3();
