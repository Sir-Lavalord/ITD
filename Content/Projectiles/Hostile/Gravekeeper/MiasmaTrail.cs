...
namespace ITD.Content.Projectiles.Hostile.Gravekeeper
{
	...
	{
		private static float Lifespan = 80f;
		private static float Halflife = 50f;
		...
		public override bool PreDraw(ref Color lightColor)  // horrible evil vanilla code
		{
			...
			Color value2 = new Color(100, 120, 255, 70);
				...
				for (int i = 0; i < 2; i++)
				{
					for (float j = 1f; j >= 0f; j -= 0.2f)
					{
						...
						Color secondaryColor = primaryColor;
							secondaryColor.G /= 2;
							secondaryColor.B /= 2;
							secondaryColor.A = (byte)Math.Min((float)primaryColor.A + 80f * num11, 255f);
