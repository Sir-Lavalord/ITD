...
public class CosmicWaterBlob : ITDProjectile
{
    //john vertexstrip!
    public override string Texture => "ITD/Content/Projectiles/Hostile/CosJel/CosmicWave";
    ...
    public override void AI()
    {
        if (Projectile.ai[1]++ < 40)
        {
            ...
            Projectile.velocity *= 1 + (Projectile.ai[0] / 100);
            Projectile.netUpdate = true;
        }
        else if (Projectile.ai[1] >= 80 && Projectile.ai[1] <= 120)
        {
            if (++Projectile.frameCounter >= 4)
            {
                Projectile.frameCounter = 0;
