...
public class WispFireOrb : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        else
        {
            ...
            {
                float desiredSpeed = 4f;
                Vector2 targetPos = player.Center;
                if (expertMode || masterMode)
                {
                    targetPos = player.Center + player.velocity * 5;
                }
                MiscHelpers.BetterEssScale(2, 0.5f);
                float targetAngle = Projectile.AngleTo(player.Center);
                Projectile.velocity = Projectile.velocity.ToRotation().AngleTowards(targetAngle, MathHelper.ToRadians(20)).ToRotationVector2() * desiredSpeed;
            }
        }
    }
}
