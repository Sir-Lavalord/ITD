...
namespace ITD.Content.Projectiles
{
    public class TheScorpionProjectile : ITDSnaptrap
    {
        ...
        public override void OneTimeLatchEffect()
        {
            ...
            float strength = (towardsPlayer.Length() / 16f)*(Main.npc[TargetWhoAmI].knockBackResist <= 0.1f? 0.1f : Main.npc[TargetWhoAmI].knockBackResist);
            Main.npc[TargetWhoAmI].velocity += normalized*strength;
            retracting = true;
        }
