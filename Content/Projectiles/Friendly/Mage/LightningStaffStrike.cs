    ...
    public override void AI()
    {
        ...
        if (Projectile.velocity == Vector2.Zero)
        {
            ...
            if (Main.rand.NextBool(Projectile.extraUpdates))
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    Vector2 vector = new((float)Math.Cos(num) * num2, (float)Math.Sin(num) * num2);
        ...
        else
        {
            ...
            while (true)
            {
                ...
                num7 %= 100;
                float f = num7 / 100f * ((float)Math.PI * 2f);
            ...
            if (Projectile.velocity != Vector2.Zero)
            {
                ...
                Projectile.localAI[0] += spinningpoint.X * (Projectile.extraUpdates + 2) * 2f * num5;
