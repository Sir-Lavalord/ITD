	...
	public class SherbetFlailProjectile : ModProjectile
	{
			...
			switch (CurrentAIState) {
				case AIState.Spinning: {
						...
						if (SpinningStateTimer % 25 == 0 && ChargeLevel < 3f) {
							...
							for (int i = 0; i < 3; i++) {
								Dust dust = Dust.NewDustDirect(player.Center, 0, 0, DustID.Firework_Pink, 0, 0, 150, default(Color), 1f + (0.5f * ChargeLevel));
								dust.noGravity = true;
								dust.velocity *= 2f;
							}
						}
		...
		public override bool OnTileCollide(Vector2 oldVelocity) {
			...
			for (int i = 0; i < 7; i++) {
				...
				Dust dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Firework_Pink, randVelocity.X, randVelocity.Y, 150, default(Color), 1f + (0.5f * ChargeLevel));
				dust.noGravity = true;
				dust.velocity *= 2f;
			}
