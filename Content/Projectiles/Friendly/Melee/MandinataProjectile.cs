    ...
    public class MandinataProjectile : ModProjectile
    {
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (Projectile.ai[1] == 0f && modPlayer.itemVar[0] == 0f)
			{
				...
				for (int i = 0; i < 10; i++)
				{
					Dust dust = Dust.NewDustDirect(player.Center, 1, 1, DustID.Torch, 0f, 0f, 110, default(Color), 3f);
					dust.velocity *= 4f;
					dust.noGravity = true;
				}
		...
		public override bool PreAI() // vanilla code of doom
		{
			...
			if (!player.frozen)
			{
				...
				Projectile.rotation = vector2.AngleTo(target) + 0.7853982f * (float)player.direction;
			...
			Dust dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height, DustID.Torch, 0f, 0f, 110, default(Color), 3f);
		...
		{
				float f3 = Projectile.rotation - 0.7853982f * (float)Math.Sign(Projectile.velocity.X) + ((Projectile.spriteDirection == -1) ? 3.14159274f : 0f);
				float num24 = 0f;
		...
		{			
			Player player = Main.player[Projectile.owner];
			Vector2 position = Projectile.position + new Vector2((float)Projectile.width, (float)Projectile.height) / 2f + Vector2.UnitY * Projectile.gfxOffY - Main.screenPosition - Vector2.UnitY * player.gfxOffY;
			Texture2D texture = ModContent.Request<Texture2D>(Texture).Value;
			Vector2 origin = new Vector2((float)texture.Width, (float)texture.Height) / 2f;
			float rotation = Projectile.rotation;
			origin = new Vector2((Projectile.spriteDirection == 1) ? ((float)texture.Width - -8f) : -8f, (player.gravDir == 1f) ? -8f : ((float)texture.Height - -8f));
