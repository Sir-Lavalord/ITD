...
public class MangledSheerthornProj : ModProjectile
{
    ...
    public override bool OnTileCollide(Vector2 oldVelocity)
    {
        if (!isStuck)
        {
            ...
            for (int i = 0; i < 1; i++)
            {
                ...
                Dust dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Copper, randVelocity.X, randVelocity.Y, 150, default, 2f);
                dust.noGravity = true;
                dust.velocity *= 2f;
            }
            isStuck = true;
    ...
    public override void OnKill(int timeLeft)
    {
        for (int i = 0; i < 5; i++)
        {
            Dust dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Copper, 0, 0, 150, default, 2f);
            dust.noGravity = true;
            dust.velocity *= 2f;
        }
        SoundEngine.PlaySound(SoundID.Dig, Projectile.Center);
