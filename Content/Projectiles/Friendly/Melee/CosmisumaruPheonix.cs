...
public class CosmisumaruPheonix : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Main.rand.NextBool(2))
        {
            ...
            if (choice == 0)
            {
                for (int d = 0; d < 2; d++)
                {
                    Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<CosJelDust>(), Projectile.velocity.X * 0.25f, Projectile.velocity.Y * 0.25f, 150, default, 0.7f);
                }
            }
            else if (choice == 1)
            {
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (int d = 0; d < 9; d++)
        {
            ...
            Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<CosJelDust>(), direction.X * speed, direction.Y * speed, 150, default, Projectile.scale);
        }
    }
}