                        ...
                        if (shouldSwitchToRetracting)
                        {
                            if (ChargeLevel >= 2)
                            {
                                if (Main.myPlayer == Projectile.owner)
                                {
                                    ...
                                    Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.oldVelocity, ModContent.ProjectileType<MoulderBoulderProj>(), (int)(Projectile.damage), Projectile.knockBack, Projectile.owner);
                                }
                            }
        ...
        public override bool OnTileCollide(Vector2 oldVelocity)
        {
            ...
            if (CurrentAIState != AIState.Retracting)
            {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    Dust dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Stone, randVelocity.X, randVelocity.Y, 150, default(Color), 1f + (0.5f * ChargeLevel));
                    dust.noGravity = true;
                    dust.velocity *= 2f;
                }
            }
