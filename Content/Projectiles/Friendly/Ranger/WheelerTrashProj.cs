...
namespace ITD.Content.Projectiles.Friendly.Ranger
{
    public class WheelerTrashProj : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                Dust dust = Main.dust[Terraria.Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, dustcolor, Projectile.velocity.X / 2, Projectile.velocity.Y / 2)];
                dust.noGravity = true;
                dust.velocity *= 0.9f;
            }
                switch (Projectile.ai[0])
