...
{
    public class VocalZeroProjectile : ModProjectile
    {
        private const string toChainTexture = "ITD/Content/Projectiles/Friendly/VocalZeroChain";
        ...
        float shootRange = 16f * 12f; // 16f*12f Means 12 tiles (each tile is 16 pixels)
        float staticRotation; //
        bool retracting = false; //
        float retractAccel = 1.8f; // Acceleration of the Snaptrap while retracting
        int framesUntilRetractable = 10;
        float extraFlexibility = 16f * 4f; // The amount of tiles you can go outside shootRange without forced retraction.
        int framesBetweenHits = 16; // 60 = 1 second between each hit
        int damageTimer = 0; //
        int minDamage = 1280; // Damage on first hit. (Most are 40% of its maxDamage but I'm keeping it customizable)
        int maxDamage = 3200; // Damage dealt at full power.
        int maxDamageStatic = 3200; // VOCALZERO SPECIFIC
        int currentDamageAmount = 0; //
        int fullPowerHitsAmount = 10;  // How many times should the Snaptrap deal damage before it's at full power?
        int warningFrames = 80; // Time it takes for the Snaptrap to retract once the warning has started
        int warningTimer = 0; //
        bool shouldBeWarning = false; //
        bool hasDoneLatchEffect = false;
        int chompDust = DustID.Blood;
        ...
        public void OneTimeLatchEffect()
        {
            ...
            PunchCameraModifier screenshake = new PunchCameraModifier(Projectile.Center, (Main.rand.NextFloat() * ((float)Math.PI * 2f)).ToRotationVector2(), 10f, 8f, 20, 1000f, FullName);
