...
{
    public class SnaptrapProjectile : ModProjectile
    {
        ...
        float shootRange = 16f*16f; // 16f*12f Means 12 tiles (each tile is 16 pixels)
        ...
        float extraFlexibility = 16f * 2f; // The amount of tiles you can go outside shootRange without forced retraction.
        int framesBetweenHits = 22; // 60 = 1 second between each hit
        int damageTimer = 0; //
        int minDamage = 20; // Damage on first hit. (Most are 40% of its maxDamage but I'm keeping it customizable)
        int maxDamage = 50; // Damage dealt at full power.
        ...
        int warningFrames = 60; // Time it takes for the Snaptrap to retract once the warning has started
        int warningTimer = 0; //
        bool shouldBeWarning = false; //
        bool hasDoneLatchEffect = false;
        int chompDust = DustID.Titanium;
        bool hasSpawnedChain = false;
        VerletChain chain;
        public override void OnSpawn(IEntitySource source)
        {
            hasSpawnedChain = false;
        ...
        private void NormalAI(Vector2 mountedCenter, float chainLength)
        {
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                {
                    bool stillInUse = !myPlayer.noItems && !myPlayer.CCed;
                    if (!stillInUse)
                    {
                        Projectile.damage = 0;
                        Projectile.tileCollide = false;
        ...
        private const int StickTime = 60 * 20; // 20 seconds,
        private void StickyAI(float chainLength)
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                if (myPlayer.HeldItem.ModItem is Snaptrap && Main.mouseRight)
                {
                    retracting = true;
                }
                if (--framesUntilRetractable <= 0)
