...
public class ITDChestTE : ModTileEntity
{
    ...
    private void DrawSlots(SpriteBatch spriteBatch)
    {
        ...
        for (int i = 0; i < StorageDimensions.X; i++)
        {
            for (int j = 0; j < StorageDimensions.Y; j++)
            {
                ...
                if (Utils.FloatIntersect(Main.mouseX, Main.mouseY, 0f, 0f, num, num2, TextureAssets.InventoryBack.Width() * Main.inventoryScale, TextureAssets.InventoryBack.Height() * Main.inventoryScale) && !PlayerInput.IgnoreMouseInterface)
                {
                    player.mouseInterface = true;
    ...
    public static void Draw(SpriteBatch spriteBatch, Item[] inv, int context, int slot, Vector2 position, Color lightColor = default)
    {
        ...
        else
        {
            switch (context)
            {
                ...
                case 13:
                    {
                        byte b = 200;
                        if (slot == Main.LocalPlayer.selectedItem)
                        {
            ...
            if (item.type == ItemID.RubblemakerSmall || item.type == ItemID.RubblemakerMedium || item.type == ItemID.RubblemakerLarge)
            {
                ...
                switch (item.type)
                {
                    case ItemID.RubblemakerSmall:
                        {
                            Texture2D value10 = TextureAssets.Extra[ExtrasID.RubbleMakerIndicator].Value;
                    ...
                    case ItemID.RubblemakerMedium:
                        {
                            Texture2D value9 = TextureAssets.Extra[ExtrasID.RubbleMakerIndicator].Value;
                    ...
                    case ItemID.RubblemakerLarge:
                        {
                            Texture2D value8 = TextureAssets.Extra[ExtrasID.RubbleMakerIndicator].Value;
        ...
        if (context == 0 && slot < 10)
        {
            ...
            int num12 = 0;
            if (Main.LocalPlayer.selectedItem == slot)
            {
