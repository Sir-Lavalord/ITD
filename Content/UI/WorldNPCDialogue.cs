...
public sealed class WorldNPCDialogueBox : ITDUIElement
{
    ...
    public static WorldNPC TalkNPC
    {
        get
        {
    ...
    {
        if (string.IsNullOrEmpty(speakerKey))
            return ITD.Instance.Assets.Request<Texture2D>("Systems/WorldNPCs/Assets/BoxStyles/DefaultBoxStyle");
...
public class DialogueButton(GeneratedButtonData buttonData) : ITDUIElement()
{
    ...
    private readonly float extraSize = 0f;
    private Keyframe<float> tweenReference;
    public void DrawButton(SpriteBatch spriteBatch, Texture2D texture, float opacity, Asset<DynamicSpriteFont> font)
    {
        ...
        if (Parent is WorldNPCDialogueBox parent)
