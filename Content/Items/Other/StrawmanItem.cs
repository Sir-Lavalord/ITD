...
public class StrawmanItem : ModItem
{
    ...
    public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            Gore g = Gore.NewGoreDirect(null, new Vector2(Main.MouseWorld.X - 15, Main.MouseWorld.Y - 30), default, Main.rand.Next(61, 64), 1f);
            g.velocity.X++;
            g.velocity.Y++;
            g = Gore.NewGoreDirect(null, new Vector2(Main.MouseWorld.X - 15, Main.MouseWorld.Y - 30), default, Main.rand.Next(61, 64), 1f);
            g.velocity.X--;
            g.velocity.Y++;
            g = Gore.NewGoreDirect(null, new Vector2(Main.MouseWorld.X - 15, Main.MouseWorld.Y - 30), default, Main.rand.Next(61, 64), 1f);
            g.velocity.X++;
