...
public class MangledSheerthorn : ModItem
{
    ...
    public override void MeleeEffects(Player player, Rectangle hitbox)
    {
        ...
        Dust.NewDustPerfect(spinPos, DustID.Copper, new Vector2?(value * 4f), 100, default, 1.5f).noGravity = true;
        if (player.itemAnimation == 1f)
        {
            ...
            if (WorldGen.ActiveAndWalkableTile(point.X, point.Y - 1) && !WorldGen.SolidTile(point.X, point.Y - 2, false))
            {
                ...
                for (int i = 0; i < 12; i++)
                {
                    Dust.NewDustPerfect(position, DustID.Copper, new Vector2(Main.rand.NextFloat() * 6f * player.direction, -8f + 8f * Main.rand.NextFloat()), 0, default, 1.5f).noGravity = true;
                }
            }
        }
    }
