...
public class Cosmisumaru : ModItem
{
    ...
    public override void HoldItem(Player player)
    {
        ...
        if (player.ITD().charge > 1499)
        {
            if (Main.rand.NextBool(3))
            {
                for (int i = 0; i < 3; i++)
                {
                    Dust.NewDust(player.position, player.width, player.height, ModContent.DustType<CosmisumaruIndicator>(), 0f, 0f, 150, default, 1.5f);
                }
            }
        }
    }
    ...
    public override bool? UseItem(Player player)
    {
        if (player.altFunctionUse == 2)
        {
            if (player.ITD().charge > 1499)
            {
                ...
                for (int d = 0; d < 30; d++)
                {
                    Dust.NewDust(player.position, player.width, player.height, ModContent.DustType<CosJelDust>(), 0f, 0f, 150, default, 1.5f);
                }
    ...
    public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
    {
        ...
        Texture2D texture = Mod.Assets.Request<Texture2D>("Content/Items/Weapons/Melee/Cosmisumaru").Value;
