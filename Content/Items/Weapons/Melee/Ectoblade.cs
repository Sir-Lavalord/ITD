...
public class Ectoblade : ModItem
{
    ...
    public override void MeleeEffects(Player player, Rectangle hitbox)
    {
        ...
        if (emitter != null)
        {
            ...
            for (int j = 0; j < 2; j++)
            {
                MiscHelpers.GetPointOnSwungItemPath(player, 60f, 60f, 0.5f + 0.6f * Main.rand.NextFloat(), player.GetAdjustedItemScale(Item), out Vector2 position, out Vector2 spinningpoint);
