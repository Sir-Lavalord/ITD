...
public class StormPlayer : ModPlayer
{
    ...
    private void CreateTornadoDust(float progress)
    {
        ...
        Vector2 offset = new((float)Math.Cos(angle) * radius, -height);
        Vector2 position = tornadoBase + offset;
        Vector2 velocity = new(-offset.X * 0.05f, Main.rand.NextFloat(-1f, 1f));
    ...
    private static void CreateElectricSpark(Vector2 position, Vector2 velocity, float progress)
    {
    ...
    private static void CreateSmokeDust(Vector2 position, Vector2 velocity, float progress)
    {
