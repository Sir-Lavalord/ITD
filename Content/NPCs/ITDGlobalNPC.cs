...
public class ITDGlobalNPC : GlobalNPC
{
    ...
    private static readonly int[] shouldDropSandberusSummon =
        [
    ...
    public override bool PreHoverInteract(NPC npc, bool mouseIntersects)
    {
        ...
        if (!mouseIntersects)
            return true;
        if (Main.mouseRight && Main.mouseRightRelease && npc.ModNPC is ITDNPC itdNPC)
            itdNPC.OnRightClick(Main.LocalPlayer);
        return true;
    }
    public override void PostDraw(NPC npc, SpriteBatch spriteBatch, Vector2 ScreenPosition, Color drawColor)
    {
        if (chilled)
        {
            ...
            Texture2D iceTexture = ITD.Instance.Assets.Request<Texture2D>("Content/Projectiles/Friendly/Melee/Snaptraps/Extra/FrostgripIceCube").Value;

            Vector2 drawOrigin = new(iceTexture.Width / 2, iceTexture.Height / 2);
