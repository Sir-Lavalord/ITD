using ITD.Content.Achievements;
...
public class CosmicJellyfishHand : ModNPC
{
    ...
    public override void AI()
    {
        ...
        Vector2 extendOut = new((float)Math.Cos(body.rotation), (float)Math.Sin(body.rotation));
                            ...
                            if (Timer == 1)
                            {
                                if (NPC.ai[3] == 1)
                                {
                                    ...
                                    if (Main.netMode != NetmodeID.MultiplayerClient)
                                    {
                                        ...
                                        ModContent.ProjectileType<CosmicJellyfishBlast>(), NPC.damage, 2f, player.whoAmI);
                                        Blast.damage = 0;
                                        Blast.ai[1] = 320f;
        ...
        for (int i = 0; i < Main.maxProjectiles; i++)
        {
            ...
            if (projectile.active && projectile.type == ProjectileID.CopperShortswordStab && Math.Abs(NPC.position.X - projectile.position.X) + Math.Abs(NPC.position.Y - projectile.position.Y) < NPC.width)
            {
                    ...
                    player.Distance(NPC.Center) < 60f)
                {
                    ...
                    // unlock the parry achievement
                    if (!Main.dedServ)
                        ModContent.GetInstance<PARRY>().ParryCondition.Complete();
                }
            }
