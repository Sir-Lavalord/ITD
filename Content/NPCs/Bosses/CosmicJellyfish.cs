...

{
    ...
    public class CosmicJellyfish : ITDNPC
    {
                                ...
                                if (AITimer1++ >= 60)
                                {
                                    ...
                                    for (int i = 0; i < count; i++)
                                    {
                                        ...
                                        Projectile sword = Projectile.NewProjectileDirect(NPC.GetSource_FromThis(), eyePos + new Vector2(dist, 0).RotatedBy(rot),
                                            Vector2.Zero, ModContent.ProjectileType<CosmicSwordStar>(), (NPC.damage), 1f, Main.myPlayer, NPC.whoAmI, 0, 1);
                                        sword.rotation = vel.ToRotation();
                                    }
                                    for (int i = 0; i < 18; i++)
                                    {
                        ...
                        if (AITimer1 >= 8)
                        {
                            if (Main.netMode != NetmodeID.MultiplayerClient)
                            {
                                ...
                                int proj = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + saferange * Vector2.UnitX.RotatedBy(rotation), Vector2.Zero,
                                    ModContent.ProjectileType<CosmicSwordStar>(), 30, 0f, -1, NPC.whoAmI, 2);
        ...
        private void ShardSlam()//slam attack, call once hit the ground
        {
            ...
            if (Main.netMode != NetmodeID.MultiplayerClient)
            {
                ...
                if (bSecondStage)
                {
                    for (int j = -1; j <= 1; j += 2)
                    {
                        Projectile shockwave = Projectile.NewProjectileDirect(NPC.GetSource_FromAI(), NPC.Center + new Vector2(30 * j, -10), new Vector2(1 * j, 0), ModContent.ProjectileType<CosmicShockwave>(), 30, 0, -1);
        ...
        private void Movement(Player player)//___________________________________________________________________________________________________________________________________________________
        {
            ...
            switch (AI_State)
            {
                ...
                case MovementState.Teleport:// flash whole body, then teleport on top of player, no contact damage
                    ...
                    {
                        Teleport(Main.player[NPC.target].Center + new Vector2(0, -400), 60, 120, (int)AttackID);
                    }
                    NPC.velocity *= 0.95f;
                    rotation = rotationFactor * maxRotation;
                    NPC.rotation = rotation;
        ...
        {
            AITimer1++;
            if (AITimer1 == time1 - 20 || AITimer1 == time2 - 20)
            {
                teleEffect = 1;
                if (!reappearTele)
                {
