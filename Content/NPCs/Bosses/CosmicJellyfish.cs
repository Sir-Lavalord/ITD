...
public class CosmicJellyfish : ITDNPC
{
                        ...
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            ...
                            for (int i = 0; i < 2; i++)
                                Projectile.NewProjectileDirect(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero,
                                    ModContent.ProjectileType<CosmicFistBump>(), NPC.damage, 0f, -1, player.whoAmI, i, dur);
                        }
                    }
                    AITimer1 = 0;
                    AttackCount++;
                ...
                if ((AITimer1 == 160 && AttackCount < finalAttack) || AITimer1 == 200)
                {
                    ...
                    if ((AttackCount % 2 == 0) && AttackCount < finalAttack - 1)
                    {
                        if (Main.netMode != NetmodeID.MultiplayerClient)
                        {
                            for (int i = 0; i < 2; i++)
                                Projectile.NewProjectileDirect(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero,
                                    ModContent.ProjectileType<CosmicFistBarrier>(), NPC.damage, 0f, -1, player.whoAmI, i);
                        }
                    }
                    AITimer1 = 0;
    ...
    private void ShardSlam()//slam attack, call once hit the ground
    {
        ...
        if (Main.netMode != NetmodeID.MultiplayerClient)
        {
            for (int i = 0; i < 10; i++)
            {
                Vector2 projectileVelo = new(startXVelo + XVeloDifference * i, -5f);
    ...
    public static VertexStrip TrailStrip = new();
