...
public class QuartzElemental : ModNPC
{
    ...
    public override void AI() // Hovering AI but evil
    {
        ...
        int posX = (int)((NPC.position.X + NPC.width / 2) / 16f) + NPC.direction * 2;
        int posY = (int)((NPC.position.Y + NPC.height) / 16f);
        bool fall = true;
        ...
        for (int i = posY; i < posY + hoverDistance; i = num + 1)
        {
            ...
            if (tile == null)
                tile = new Tile();
            if ((!tile.IsActuated && Main.tileSolid[tile.TileType]) || tile.LiquidAmount > 0)
            {
                fall = false;
                break;
            }
        ...
        if (((Main.player[NPC.target].position - NPC.position).Length() < 200 && Collision.CanHit(NPC.position, NPC.width, NPC.height, Main.player[NPC.target].position, Main.player[NPC.target].width, Main.player[NPC.target].height)) || NPC.ai[3] >= 32f)
        {
            ...
            if (NPC.ai[3] >= 64f)
            {
                ...
                for (int j = 0; j < 8; ++j)
                {
                    int dust = Dust.NewDust(NPC.Center, 0, 0, DustID.GiantCursedSkullBolt, 0f, 0f, 100, default, 2f);
