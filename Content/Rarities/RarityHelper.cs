...
{
    public static void DrawBaseTooltipTextAndGlow(Vector2 pos, Vector2 origin, Vector2 scale, float rotation, float maxWidth, string text, Color glowColor, Color textOuterColor, Color? textInnerColor = null, Texture2D glowTexture = null, Vector2? glowScaleOffset = null)
    {
        Main.spriteBatch.End(out SpriteBatchSnapshot spriteBatchData); // restart spritebatch
        ...
        Vector2 textPosition = new(pos.X, pos.Y);
        Vector2 glowPosition = new Vector2(pos.X + textCenter.X, pos.Y + textCenter.Y / 1.5f) - origin;
        Vector2 glowScale = new Vector2(textSize.X * 0.115f, 0.6f) * glowScaleOffset.Value * scale;
        glowColor.A = 0;
        ...
        for (int i = 0; i < 12; i++)
        {
            ...
            ChatManager.DrawColorCodedString(Main.spriteBatch, FontAssets.MouseText.Value, text, (textPosition + afterimageOffset).RotatedBy(MathHelper.TwoPi * (i / 12)), textOuterColor * 0.9f, rotation, origin, scale);
        }

        // Draw the main inner text.
        Color mainTextColor = Color.Lerp(glowColor, textInnerColor.Value, 0.9f);
        ChatManager.DrawColorCodedString(Main.spriteBatch, FontAssets.MouseText.Value, text, textPosition, mainTextColor, rotation, origin, scale, maxWidth);
