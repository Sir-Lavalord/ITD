...
public class WavyHandler : ITagHandler
{
    TextSnippet ITagHandler.Parse(string text, Color baseColor, string options)
    {
        ...
        {
            if (float.TryParse(paramsStrings[0], out float ampl0))
                ampl = ampl0;
            if (paramsStrings.Length > 1 && float.TryParse(paramsStrings[1], out float freq0))
                freq = freq0;
        }
...
public class WavySnippet : TextSnippet
{
    ...
    private readonly float[] yOffsets;
