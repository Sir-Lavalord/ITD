...
public static class NPCHelpers
{
    public static void StepUp(ref Vector2 position, ref Vector2 velocity, int width, int height)
    {
        ...
        vector.X += velocity.X;
        int num2 = (int)((vector.X + width / 2 + (width / 2 + 1) * num) / 16f);
        int num3 = (int)((vector.Y + height - 1f) / 16f);
        ...
        for (int j = 2; j < num4 + 1; j++)
        {
            ...
            flag = flag && (!tile.HasUnactuatedTile || !Main.tileSolid[tile.TileType] || Main.tileSolidTop[tile.TileType]);
        }
        tile = Main.tile[num2 - num, num3 - num4];
        flag2 = flag2 && (!tile.HasUnactuatedTile || !Main.tileSolid[tile.TileType] || Main.tileSolidTop[tile.TileType]);
        bool flag3 = true;
        bool flag4 = true;
        Tile tile2;
        ...
        flag3 = flag3 && (!tile.HasUnactuatedTile || !Main.tileSolid[tile.TileType] || Main.tileSolidTop[tile.TileType] || (tile.Slope == SlopeType.SlopeDownLeft && position.X + width / 2 > num2 * 16) || (tile.Slope == SlopeType.SlopeDownRight && position.X + width / 2 < num2 * 16 + 16) || (tile.IsHalfBlock && (!tile2.HasUnactuatedTile || !Main.tileSolid[tile2.TileType] || Main.tileSolidTop[tile2.TileType])));
        tile = Main.tile[num2, num3];
        tile2 = Main.tile[num2, num3 - 1];
        flag4 = flag4 && ((tile.HasUnactuatedTile && (!tile.TopSlope || (tile.Slope == SlopeType.SlopeDownLeft && position.X + width / 2 < num2 * 16) || (tile.Slope == SlopeType.SlopeDownRight && position.X + width / 2 > num2 * 16 + 16)) && (!tile.TopSlope || position.Y + height > num3 * 16) && Main.tileSolid[tile.TileType] && !Main.tileSolidTop[tile.TileType]) || (tile2.IsHalfBlock && tile2.HasUnactuatedTile));
        flag4 &= !Main.tileSolidTop[tile.TileType] || !Main.tileSolidTop[tile2.TileType];
        if (num2 * 16 < vector.X + width && num2 * 16 + 16 > vector.X)
        {
            if (flag4 & flag3 & flag & flag2)
            {
                ...
                if (num5 < vector.Y + height)
                {
                    float num6 = vector.Y + height - num5;
                    if ((double)num6 <= 16.1)
                    {
                        position.Y = num5 - height;
                        return;
                    }
                }
            }
