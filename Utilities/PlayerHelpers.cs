...
{
    public static void HurtCustom(this Player p, string key, int damage, int hitDirection = 0)
    {
        p.Hurt(p.DeathByLocalization(key), damage, hitDirection);
    }
    ...
    public static Player FromGuid(Guid guid) => Main.player.FirstOrDefault(p => p.active && p.ITD().guid == guid);
    public static ITDPlayer ITD(this Player player) => player.GetModPlayer<ITDPlayer>();
    public static SnaptrapPlayer Snaptrap(this Player player) => player.GetModPlayer<SnaptrapPlayer>();
    ...
    public static Vector2 LookDirection(this Player player) => (player.ITD().MousePosition - player.Center).SafeNormalize(Vector2.Zero);
    ...
    public static void AddBuff<T>(this Player p, int timeToAdd, bool quiet = true, bool foodHack = false) where T : ModBuff
        => p.AddBuff<T>(timeToAdd, quiet, foodHack);
}
