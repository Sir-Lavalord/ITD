...
public sealed class FrontGunLayer : PlayerDrawLayer
{
    ...
    protected override void Draw(ref PlayerDrawSet drawInfo)
    {
        ...
        Color drawColor = Lighting.GetColor((int)(drawInfo.Position.X + drawPlayer.width * 0.5) / 16, (int)((drawInfo.Position.Y + drawPlayer.height * 0.5) / 16.0));

        Vector2 position = new Vector2((int)(drawInfo.Position.X - drawPlayer.bodyFrame.Width / 2 + drawPlayer.width / 2), (int)(drawInfo.Position.Y + drawPlayer.height - drawPlayer.bodyFrame.Height + 4f)) + drawPlayer.bodyPosition + new Vector2(drawPlayer.bodyFrame.Width / 2, drawPlayer.bodyFrame.Height / 2) - new Vector2(drawPlayer.direction * 5f, 0f);//Player.MountedCenter - new Vector2(Player.direction*4f, Player.gravDir*2f);
        ...
        Vector2 offset = heldItem.ModItem.HoldoutOffset() ?? new Vector2();

        DrawData drawData = new(texture, position, sourceRectangle, drawColor, rotation - modPlayer.recoilFront * drawPlayer.direction * drawPlayer.gravDir, origin + new Vector2((offset.X - 4f) * drawPlayer.direction, (offset.Y - 4f) * drawPlayer.gravDir), adjustedItemScale, effects, 0f);
...
public sealed class BackGunLayer : PlayerDrawLayer
{
    ...
    protected override void Draw(ref PlayerDrawSet drawInfo)
    {
        ...
        Vector2 position = new Vector2((int)(drawInfo.Position.X - drawPlayer.bodyFrame.Width / 2 + drawPlayer.width / 2), (int)(drawInfo.Position.Y + drawPlayer.height - drawPlayer.bodyFrame.Height + 4f)) + drawPlayer.bodyPosition + new Vector2(drawPlayer.bodyFrame.Width / 2, drawPlayer.bodyFrame.Height / 2) + new Vector2(drawPlayer.direction * 6f, drawPlayer.gravDir * 2f);//Player.MountedCenter - new Vector2(Player.direction*4f, Player.gravDir*2f);
        position -= Main.screenPosition;
        ...
        DrawData drawData = new(texture, position, sourceRectangle, drawColor, rotation - modPlayer.recoilBack * drawPlayer.direction * drawPlayer.gravDir, origin + new Vector2(offset.X * drawPlayer.direction, offset.Y * drawPlayer.gravDir), adjustedItemScale, effects, 0f);
        drawInfo.DrawDataCache.Add(drawData);
    }
}