...
public class ITDPlayer : ModPlayer
{
    ...
    public bool SetAlloy => setAlloy_Melee || setAlloy_Ranged || setAlloy_Magic;
    ...
    {
        shakeDuration = dur;
        shakeIntensityX = powerX;
        shakeIntensityY = powerY;
        shakeDecay = Decay;
    }
    ...
    public override void MeleeEffects(Item item, Rectangle hitbox)
    {
        if (wickedHeart)
        {
            if (Player.HeldItem.DamageType == DamageClass.Melee && wickedHeartEffect && wickedHeartCooling < 1)
            {
                ...
                Projectile.NewProjectileDirect(Player.GetSource_FromThis(), Player.position, shootTowardsMouse,
    ...
    public override void OnConsumeMana(Item item, int manaConsumed)
    {
        if (Player.HeldItem.DamageType == DamageClass.Magic && wickedHeartEffect && wickedHeartCooling < 1)
        {
            ...
            Projectile.NewProjectileDirect(Player.GetSource_FromThis(), Player.Center, shootTowardsMouse,
    ...
    public override void OnHitAnything(float x, float y, Entity victim)
    {
        if (wickedHeart)
        {
            ...
            {
                Projectile.NewProjectileDirect(Player.GetSource_FromThis(), victim.Center, Vector2.Zero,
    ...
    private sealed class ITDPlayerSystem : ModSystem // there's no OnExitWorld hook :death:
    {
        ...
        public override void PostDrawTiles()
        {
            ...
            Asset<Texture2D> tex = Mod.Assets.Request<Texture2D>("Content/SelectBox");
