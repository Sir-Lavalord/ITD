...
namespace ITD.Effects
{
    public class EoWHealthbar : ModResourceOverlay
    {
        ...
        {
            context.texture = heartTexture ??= ModContent.Request<Texture2D>("ITD/Effects/ClassicLifeOverlay");
            context.Draw();
        }
        ...
        private void DrawFancyPanelOverlay(ResourceOverlayDrawContext context)
        {
            ...
            context.texture = fancyPanelTexture ??= ModContent.Request<Texture2D>("ITD/Effects/FancyLifeOverlay_Panel");
        ...
        private void DrawBarsOverlay(ResourceOverlayDrawContext context)
        {
            ...
            context.texture = barsFillingTexture ??= ModContent.Request<Texture2D>("ITD/Effects/BarsLifeOverlay_Fill");
            context.Draw();
        }
        private void DrawBarsPanelOverlay(ResourceOverlayDrawContext context)
        {

            context.texture = barsPanelTexture ??= ModContent.Request<Texture2D>("ITD/Effects/BarsLifeOverlay_Panel");
            context.source = context.texture.Frame();
